...
import { Mail, Lock, User, ArrowRight, ChevronLeft, AlertCircle } from 'lucide-react';
...
export default function AuthPage({ onLoginSuccess, onBack }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        // Auto-derive username if not explicitly set
        const username = formData.username || formData.email.split('@')[0].replace(/[^a-zA-Z0-9_]/g, '_');
        
        const res = await fetch(`${HTTP_BASE_URL}/api/v1/auth/signup`, {
          method: 'POST',
        ...
        const res = await fetch(`${HTTP_BASE_URL}/api/v1/auth/login`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      <main className="w-full max-w-md my-auto py-8">
        ...
        >
          <div className="p-6 sm:p-8 space-y-6 bg-white dark:bg-black rounded-3xl overflow-hidden border border-slate-200 dark:border-transparent shadow-xl dark:shadow-none">
            ...
            {/* Error Banner */}
            {error && (
              <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/40 text-red-500 text-xs font-mono flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <>
                  <div className="space-y-1 text-left">
                    ...
                    <div className="relative">
                      <User className="w-4 h-4 text-slate-400 dark:text-slate-400 absolute left-3.5 top-3.5" />
                      <input
                        type="text"
                        name="name"
                        required
              ...
              <div className="space-y-1 text-left">
                ...
                <div className="relative">
                  <Mail className="w-4 h-4 text-slate-400 dark:text-slate-400 absolute left-3.5 top-3.5" />
                  <input
                    type="text"
                    name="email"
                    required
              ...
              <div className="space-y-1 text-left">
                ...
                <div className="relative">
                  <Lock className="w-4 h-4 text-slate-400 dark:text-slate-400 absolute left-3.5 top-3.5" />
                  <input
                    type="password"
                    name="password"
                    required
